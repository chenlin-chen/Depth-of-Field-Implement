...
def decomposeBlur(img: np.ndarray, depth: np.ndarray, focal_depth:int, focal_len:int, max_disk_r:int) -> \
    np.ndarray:
    ...
    focal_d0 = max(focal_depth - focal_len - focal_depth_disp, 0)
    focal_d1 = min(focal_depth + focal_len + focal_depth_disp, 255)
    ...
    def doBlur(premultiplied_img:np.ndarray, alpha:np.ndarray, layer_depth) -> np.ndarray:
        depth_r = max(abs(layer_depth - focal_depth)-focal_len, 0) / max_depth_disp
...
def main(args):
    ...
    focal_len = int( args.focal_len / (1.0 + (focal_depth/128.0)**2 ) )
    blured_img = decomposeBlur(sub_img, sub_depth_map_int, focal_depth, focal_len, sub_blur_r)
    blured_img = cv2.resize(blured_img, (w, h))

    focal_region = np.abs(depth_map - focal_depth) <= focal_len
    blured_img[focal_region] = img[focal_region]

    output_folder = "./outputs"
    img_name = os.path.splitext(os.path.basename(img_path))[0]
    save_name = f"{img_name}_LayerBlurR_{blur_r}_focalD{focal_depth}_focalL{focal_len}.png"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--focal_len", type=float, default=25.0, help='Focal region length (focal tolerance)')
